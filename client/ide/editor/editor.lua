...
function Editor.new(lines, width, height)
	local self = setmetatable({}, Editor)

	self.lines = lines
	self.readOnly = {}
	for i = 1, #lines do
		self.readOnly[i] = false
	end
...
function Editor:moveCursorToEndOfLine()
	...
	elseif length - self.scroll.x < width then
...
function Editor:pageUp()
	local y = math.max(1, self.cursor.y + self.scroll.y - self.height)
	self:moveCursorTo(1, y)
end
...
function Editor:pageDown()
	local y = math.min(#self.lines, self.cursor.y + self.scroll.y + self.height)
	self:moveCursorTo(1, y)
end
...
function Editor:setReadOnly(readOnly, startLine, endLine)
	for i  = startLine or 1, endLine or startLine or #self.lines do
		self.readOnly[i] = readOnly
	end
end

--- Remove a line from the editor
function Editor:removeLine(y)
	table.remove(self.lines, y)
	table.remove(self.readOnly, y)
end

--- Insert a line into the editor
function Editor:addLine(y, line)
	table.insert(self.lines, y, line)
	table.insert(self.readOnly, y, false)
end
...
function Editor:backspace()
	...
	if self.cursor.x > 1 or self.scroll.x > 0 or self.cursor.y > 1 or self.scroll.y > 0 then
		...
		if self:isReadOnly(y) then return nil end
		...
		if x > 1 then
			...
			return character
		elseif not self:isReadOnly(y - 1) then
			-- Remove the line
			local length = #self.lines[y - 1]
...
function Editor:forwardDelete()
	...
	if x <= length then
		local character = self.lines[y]:sub(1, x)
		...
		return character
	elseif y < #self.lines and not self:isReadOnly(y + 1) then
...
function Editor:gutterSize()
	return #tostring(#self.lines) + #Theme["gutter separator"]
end
...
function Editor:currentLineLength()
	return #self.lines[self.cursor.y + self.scroll.y]
end
