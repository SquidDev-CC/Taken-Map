...
local Content = require "client.ide.editor.content"
...
function TabBar.sanitiseName(name)
	...
	if #new > TabBar.maxTabWidth then
...
function TabBar:setup(delegate)
	...
	self.win = Window.new(1, TabBar.y, w, 1, term.native())
	self.current = 1
end
...
function TabBar:draw()
	...
	local win = self.win
	win
		:setBackgroundColor(Theme["tab bar background"])
		:setTextColor(Theme["tab bar text focused"])
		:clear()
		:setCursorPos(1, 1)
	...
	for i, name in pairs(names) do
		...
		win
			:setBackgroundColor(Theme["tab bar background blurred"])
			:setTextColor(Theme["tab bar text blurred"])
			:write(" ")

		if i == self.current then
			win
				:setBackgroundColor(Theme["tab bar background focused"])
				:setTextColor(Theme["tab bar text focused"])
		end

		win:write(actualName .. " ")
	end
end
...
function TabBar:determineClickedTab(x, y)
	...
	if y == 1 then
		...
		for i, name in pairs(names) do
			local actualName = TabBar.sanitiseName(name)
			local endX = currentX + #actualName - 1
