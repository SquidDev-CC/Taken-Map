...
local TextField = require "client.ide.ui.textfield"
...
function FileDialogue:show()
	...
	local win = Window.new(x, y, w, self.height, term.native())

	-- Create and clear the window
	win
		:redirectTo()
		:setCursorBlink(false)
		:setBackgroundColor(Theme["file dialogue background"])
		:clear()

	-- Navigation bar
	win
		:setCursorPos(1, 1)
		:setBackgroundColor(Theme["file dialogue close background"])
		:setTextColor(Theme["file dialogue close text"])
		:clearLine()
		:write("x")

	-- Title text
	win
		:setTextColor(Theme["file dialogue text"])
		:setBackgroundColor(Theme["file dialogue background"])
		:setCursorPos(2, 2)
		:write(self.title)

	-- Text field
	local field = TextField.new(2, 4, Theme["file dialogue text"], Theme["file dialogue background"])
	field:setWidth(-2)
	...
	field:setCallback(function(event)
		if event[1] == "mouse_click" then
			...
			if cx > 1 and cy >= listStartY and cy < listStartY + selfCopy:listHeight() then
				local item = selfCopy.listCache[cy - listStartY + 1 + self.scroll]
				if item then
					local result = selfCopy:clickOnItem(item)
					selfCopy:drawFileList()
					return result
				end
			elseif cx == 1 and cy == y then
				return false
			elseif cx == 1 then
